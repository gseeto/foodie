<?php
	 ...
	 * listing a collection of Recipe objects.  It includes
	 ...
	 * 
	 */
	class RecipeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Recipe datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Recipe object, or it can be any QQNode extending from QQN::Recipe().
		 * 
		 ...
		 * sorting using two fields from the Recipe object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Recipe
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Recipe::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Recipe, given the clauses above
			$this->DataSource = Recipe::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Recipe-based QQNode.
		 * It will also verify that it is a proper Recipe-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'recipe') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "recipe".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Recipe()->Id;
				case 'Name': return QQN::Recipe()->Name;
				case 'Description': return QQN::Recipe()->Description;
				case 'Instructions': return QQN::Recipe()->Instructions;
				case 'PreparationTime': return QQN::Recipe()->PreparationTime;
				case 'ServingSize': return QQN::Recipe()->ServingSize;
				case 'OwnerId': return QQN::Recipe()->OwnerId;
				case 'Owner': return QQN::Recipe()->Owner;
				case 'MealTypeId': return QQN::Recipe()->MealTypeId;
				default: throw new QCallerException('Simple Property not found in RecipeDataGrid content: ' . $mixContent);
