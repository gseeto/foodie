<?php
	 ...
	 * listing a collection of Quality objects.  It includes
	 ...
	 * 
	 */
	class QualityDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Quality datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Quality object, or it can be any QQNode extending from QQN::Quality().
		 * 
		 ...
		 * sorting using two fields from the Quality object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Quality
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Quality::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Quality, given the clauses above
			$this->DataSource = Quality::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Quality-based QQNode.
		 * It will also verify that it is a proper Quality-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'quality') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "quality".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Quality()->Id;
				case 'Rating': return QQN::Quality()->Rating;
				default: throw new QCallerException('Simple Property not found in QualityDataGrid content: ' . $mixContent);
