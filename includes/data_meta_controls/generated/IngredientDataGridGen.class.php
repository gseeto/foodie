<?php
	 ...
	 * listing a collection of Ingredient objects.  It includes
	 ...
	 * 
	 */
	class IngredientDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Ingredient datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Ingredient object, or it can be any QQNode extending from QQN::Ingredient().
		 * 
		 ...
		 * sorting using two fields from the Ingredient object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Ingredient
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Ingredient::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Ingredient, given the clauses above
			$this->DataSource = Ingredient::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Ingredient-based QQNode.
		 * It will also verify that it is a proper Ingredient-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ingredient') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ingredient".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Ingredient()->Id;
				case 'Name': return QQN::Ingredient()->Name;
				case 'UnitOfMeasurementId': return QQN::Ingredient()->UnitOfMeasurementId;
				case 'InitialAmount': return QQN::Ingredient()->InitialAmount;
				case 'Cost': return QQN::Ingredient()->Cost;
				case 'CostUnitOfMeasurement': return QQN::Ingredient()->CostUnitOfMeasurement;
				default: throw new QCallerException('Simple Property not found in IngredientDataGrid content: ' . $mixContent);
