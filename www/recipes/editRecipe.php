...
class EditRecipeForm extends FoodieForm {
	...
	protected function Form_Create() {
		...
		$this->txtRecipeName->Text = $this->objRecipe->Name;
		...
		$this->txtRecipeDescription->Text = $this->objRecipe->Description;
		...
		$this->txtInstructions->Text = $this->objRecipe->Instructions;
	 	...
	 	foreach(MealType::$NameArray as $key => $strMealType) {
	 		...
	 		if ($this->objRecipe->MealTypeId == $key)
	 			$btnMealType->Checked = true;
	 	...
	 	foreach(Category::LoadAll() as $objCategory) {
	 		...
	 		if($this->objRecipe->IsCategoryAssociated($objCategory))
	 	...
	 	$this->btnSubmit->Text = 'Update Recipe';
	 	$this->btnSubmit->AddAction(new QClickEvent(), new QAjaxAction('btnSubmitRecipe'));
	 	
	 	// Define the AddIngredientWidget example. passing in the Method Callback for whenever the AddIngredient dialog is Closed
...
EditRecipeForm::Run('EditRecipeForm');
?>